...
struct rk817_battery_device {
	...
	bool				hybrid_calibrated;	/* at least one V_FULL learned */
...
/*
 * The kernel worker cannot depend on the root filesystem being available in
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	return sysfs_emit(buf, "%d %d %d\n",
			  READ_ONCE(battery->hybrid_v_full_chg),
			  READ_ONCE(battery->hybrid_v_full_dis),
			  READ_ONCE(battery->hybrid_calibrated));
}

static ssize_t rk817_calibration_store(struct device *dev,
				       ...
				       const char *buf, size_t count)
{
	...
	WRITE_ONCE(battery->hybrid_v_full_dis, vdis);
	WRITE_ONCE(battery->hybrid_calibrated, true);
	BAT_INFO("hybrid: restored vfull_chg=%d vfull_dis=%d\n",
		 vchg, vdis);

	return count;
}
static DEVICE_ATTR_RW(rk817_calibration);

static struct attribute *rk817_calibration_attrs[] = {
	&dev_attr_rk817_calibration.attr,
	NULL,
};

static const struct attribute_group rk817_calibration_group = {
	.attrs = rk817_calibration_attrs,
};
